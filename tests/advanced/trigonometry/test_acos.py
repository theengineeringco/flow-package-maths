...
def test_acos(settings_in: str, data_in: FlowType, result: float) -> None:
    ...
    outport_data = ComponentTest("flow_package_maths/advanced/trigonometry/acos").run(inport_data, setting_values)

    assert outport_data["result"] == pytest.approx(base.Double(result), abs=1e-4)


if __name__ == "__main__":
    test_acos("degrees", base.Double(0.5), 60)
