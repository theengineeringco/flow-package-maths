...
def cos_function(use_values: dict = {"port1": 1}):
...
def process(component: Component):
    ...
    # actually run the cos function with the input. You can write the function explicitly instead but
    # we leverage the "call_function_with_data" as it works well for maths functions
    use_values, the_result = call_function_with_data(get_data_arr, cos_function)

    if component.debug is True:
        print("{0} is {1}".format(inports, use_values))
        print("The Result of cos(n) is {0} ".format(the_result))
