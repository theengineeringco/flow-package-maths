...
definition = {
    "name": "range",
    "description": "Get the range of an entire array of numbers.",
...
def process(component: Component):
    ...
    if component.debug:
        component.log(log_level=LogLevel.DEBUG, message=f"Calculating the range of {array}")

    # We are using numpy's built in functions so we don't have to worry
    result: float = np.ptp(array)

    if component.debug:
        component.log(log_level=LogLevel.DEBUG, message=f"The range is {result} ")
